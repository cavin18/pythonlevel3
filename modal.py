...
class TestModal(ui.Modal, title='Judul Test'):
    ...
    field_1 = ui.TextInput(label='Nama')
    field_2 = ui.TextInput(label='Alamat', style=TextStyle.paragraph)

    # Metode yang dipanggil saat modal dikirim
    async def on_submit(self, interaction: discord.Interaction):
        # Memperbarui pesan dengan data yang dimasukkan
        await interaction.message.edit(content=f'Nama: {self.field_1.value}\n'
                                             f'Alamat: {self.field_2.value}')
